 ...
 * @since 1.0.0
 */
class BQW_Grid_Accordion_Activation {
	...
	private static function single_activate() {
		global $wpdb;
		$prefix = $wpdb->prefix;
		$table_name = $prefix . 'gridaccordion_accordions';

		require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );

		// when the grid is activated for the first time, the tables don't exist, so we need to create them
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$table_name'" ) != $table_name ) {
			$create_accordions_table = "CREATE TABLE ". $prefix . "gridaccordion_accordions (
			...
			$create_panels_table = "CREATE TABLE ". $prefix . "gridaccordion_panels (
				id mediumint(9) NOT NULL AUTO_INCREMENT,
			...
			$create_layers_table = "CREATE TABLE ". $prefix . "gridaccordion_layers (
				id mediumint(9) NOT NULL AUTO_INCREMENT,
				accordion_id mediumint(9) NOT NULL,
